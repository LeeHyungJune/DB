...
*/
SELECT
    EMANE 사원이름, HIREDATE 입사일, DEPTNO 부서번호
FROM
    EMP
WHERE
    ENAME LIKE '%N'
...
*/
SELECT
    EMPNO 사원번호, ENAME 사원이름, JOB 직급, HIREDATE 입사일, SAL 급여
FROM
    EMP
WHERE
    ENAME NOT LIKE '%a%'
...
*/
SELECT
    LPAD(SUBSTR(ENAME,-2),LENGTH(ENAME),'#') "사원 대체이름",
    HIREDATE 입사일,
    SAL 급여
FROM
    EMP
...
*/
SELECT
    CONCAT(CONCAT(LPAD(SUBSTR(SUBSTR('jennie@githrd.com',1,3),3,1),3,'*'),
    LPAD(SUBSTR(SUBSTR('jennie@githrd.com',4,4),4,1),4,'*')),
    LPAD(SUBSTR(SUBSTR('jennie@githrd.com',8),-4),10,'*')) 제니
FROM
    DUAL
;

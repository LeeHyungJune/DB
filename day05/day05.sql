...
--  사원들을 조회하는데 시장부터 계층으로 조회하시오
SELECT
    LPAD(' ', (LEVEL -1)*3, '  ') || EMPNO|| '-' || ENAME || '-' ||
    JOB || '-' || MGR || '-' || LEVEL 정보, HIREDATE
FROM
    EMP
START WITH
    MGR IS NULL
...
-- EMP1 테이블에 리사 사원을 추가하시오.
INSERT INTO
    EMP1(ENAME, EMPNO)
VALUES( --  1003, '리사'  --  순서가 맞지 않아서 입력오류
    '리사', 1003
);
...
-- DELETE EMP1 WHERE EMPNO = '1002';

INSERT INTO
    EMP1    --  = EMP1(EMPON, ENAME, JOB, MGR, HIREDATE, SAL, COMM, DEPTNO)
VALUES(
    1005, '둘리'
)
;
...
--  이때 여러 개의 필드를 동시에 질의명령의 결과로 대체하고자 한다면
...
--  로제의 직급과 급여를 EMP 테이블의 SMITH 사원의데이터를 복사해서 수정하시오
UPDATE
    EMP2
SET 
    (JOB, SAL) = (
...
/*
    참고 )
      ...
      정규화 과정
        ...
        2.  제2정규화
            ==> 기본키에 대해서 모든 키는 완전함수 종속이어야 한다. (하나 알면 한 행 다 꺼내오는 것)

...
ADD(
    GEN CHAR(1)
        CONSTRAINT AVATAR_GEN_CK CHECK (GEN IN ('F','M'))
        CONSTRAINT AVATAR_GEN_NN NOT NULL
);

ALTER TABLE MEMB
DROP CONSTRAINT MEMB_ADDR_NN;

-- 아바타 테이블 데이터 추가
INSERT INTO 
    avatar(ANO, ANAME, ORINAME, SAVENAME, DIR, LEN, GEN, ADATE, ISSHOW)
VALUES(
...
INSERT INTO
    memb(mno, name, id, pw, mail, tel, avt, gen)
VALUES(
...
INSERT INTO
    memb(mno, name, id, pw, mail, tel, gen)
VALUES(
...
CREATE TABLE guestboard(
    ...
    writer NUMBER(4)
        CONSTRAINT GBRD_WRITER_FK REFERENCES memb(mno)
...
FROM
    (SELECT mno, id, name, mail FROM memb) 
